<template>
  <v-container
    ...
    elevation="0"
    :class="previewOnly ? `` : `report-print`"
    class="card-font-color"
  >
    <v-row class="pa-5">
      <v-col cols="6">
      ...
      <!-- <v-col cols="12">
        <div class="text-center">
          <h4 class="uppercase-text tb-border">
            <v-container>
              {{ payload.equipment_category.name }} Preventive Maintenance
              Service Report
            </v-container>
          </h4>
        </div>
    ...
    <v-row no-gutters>
      <v-col cols="12">
        <div class="text-center">
          <h4 class="uppercase-text tb-border py-2">
            {{ payload.equipment_category.name }} Preventive Maintenance Service
            Report
          </h4>
        </div>
      </v-col>
      <v-col cols="12" class="mx-2">
        <v-row no-gutters>
          <v-col cols="4"> Date: {{ payload.date }} </v-col>
          <v-col cols="4">
            AMC Agreement Number: <b>A{{ payload.amc.contract.id }}</b>
          </v-col>
          <v-col cols="4">
            Service Report Number:
            <span class="red--text">{{ payload.id }}</span>
          </v-col>
        </v-row>
      </v-col>
    </v-row>
        ...
        <v-container>
          <v-row no-gutters>
            <v-col cols="12" class="bottom-border">
              Company Name: <b>{{ payload.amc.contract.company.name }}</b>
            </v-col>
            <v-col cols="6" class="bottom-border">
              License Issuer:
              <b>{{ payload.amc.contract.company.trade_license.issued_by }}</b>
            </v-col>
            <v-col cols="6" class="bottom-border">
              License Number:
              <b>{{ payload.amc.contract.company.trade_license.license_no }}</b>
            </v-col>
            <v-col cols="12" class="bottom-border">
              Address:{{ payload.amc.contract.company.address }}
            </v-col>
            <v-col cols="4" class="bottom-border"> LPO Number: </v-col>
            <v-col cols="4" class="bottom-border">
              AMC Since: {{ payload.amc.contract.show_start_date }}
            </v-col>
            <v-col cols="4" class="bottom-border">
              AMC Expire: {{ payload.amc.contract.show_expire_date }}
            </v-col>
          </v-row>
        </v-container>
      </v-col>
    ...
    <v-row class="pb-3">
      <v-col>
        <table class="table-font-color">
          <thead>
            <tr>
              <th
                ...
                colspan="6"
              >
                CheckList: {{ payload.equipment_category.name }}
              </th>
            </tr>
            <tr>
          ...
          <tbody>
            <tr v-for="(item, i) in payload.checklists" :key="i">
...
export default {
  ...
  created() {
    this.attachments = this.payload.checklists
